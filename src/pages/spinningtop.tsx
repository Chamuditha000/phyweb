...
export function SpinningTop() {
  ...
  useFrame(({ clock }) => {
    ...
    if (antennaRef.current) antennaRef.current.rotation.y = t * 2;
    ...
    if (laserRef.current) laserRef.current.scale.y = 1 + Math.sin(t * 8) * 0.3;
    if (lightRef.current)
  ...
  return (
    <group ref={groupRef} rotation={[Math.PI, 0, 0]} scale={0.5} castShadow>
      ...
      <Text
        ...
        fontSize={0.18}
        color="#00ffff"
        // Load font from HTTPS so it works under file://
        font="https://unpkg.com/three@0.158.0/examples/fonts/helvetiker_bold.typeface.json"
        outlineWidth={0.004}
        outlineColor="#00ffff"
        outlineOpacity={0.6}
